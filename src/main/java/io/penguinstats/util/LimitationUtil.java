...
public class LimitationUtil {
	...
	public boolean checkDrops(List<Drop> drops, String stageId, long timestamp) {
		...
		//		Bounds itemTypeBounds = new Bounds(0, 0);
		//		Map<String, Bounds> itemQuantityBoundsMap = new HashMap<>();
		//		for (int i = 0, l = limitations.size(); i < l; i++) {
		//			Limitation limitation = limitations.get(i);
		//			if (limitation.getItemId() == null)
		//				itemTypeBounds = itemTypeBounds.simpleCombine(limitation.getBounds());
		//			else {
		//				itemQuantityBoundsMap.put(limitation.getItemId(), itemQuantityBoundsMap
		//						.getOrDefault(limitation.getItemId(), new Bounds(0, 0)).simpleCombine(limitation.getBounds()));
		//			}
		//		}
		//
		//		// check item type num
		//		if (!itemTypeBounds.isValid(typesNum))
		//			return false;
		//
		...
		//		Set<Entry<String, Bounds>> itemQuantityBoundsEntrySet = itemQuantityBoundsMap.entrySet();
		//		Iterator<Entry<String, Bounds>> iter = itemQuantityBoundsEntrySet.iterator();
		//		boolean hasSpecialTimepoint = false;
		//		while (iter.hasNext()) {
		//			Entry<String, Bounds> entry = iter.next();
		//			Drop drop = dropsMap.get(entry.getKey());
		//			int quantity = drop == null ? 0 : drop.getQuantity();
		//			if (!entry.getValue().isValid(quantity))
		//				return false;
		//		}
